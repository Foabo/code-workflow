...
Flowflow 是面向 coding harness 的工作流工具。它给一个仓库提供共享的任务事实、agent 原生工作流动作、简洁的项目基线文件，以及可预测的 helper 操作，让 Claude Code、Codex、Cursor、OpenCode、Pi 等工具可以沿用同一套开发流程。Flowflow 不试图把这些工具统一成一个大型 harness 管理器。

## 目标

Flowflow 帮助 coding agent 把一个模糊请求推进到可负责地完成任务，中间不丢失任务约定、实现计划、验证状态和可复用的项目知识。
...
Trellis、OpenSpec、Superpowers、GSD、Gentle-AI、Oh My OpenCode Slim 等框架可以作为参考，影响 adapter 或启发式规则，但不能定义 Flowflow 的运行时和 `.ff` 标准格式。

## 调用模型

日常使用发生在 coding harness 原生的 agent 命令里。生成的 agent 命令统一使用 `ff-` 前缀，避免和其他插件或命令冲突：
...
Git 仍然是代码改动的事实来源。Flowflow 只保存工作流状态、任务意图、计划、清单进度和项目基线知识。
...
`ff init` 只创建简短模板，不会在初始化时尝试完整理解一个新项目。
...
```text
ff init -> ff-understand -> ff-work
```

`ff-understand` 先写草稿，再由用户确认哪些内容合并进 `.ff/project/*`。自动扫描结果不能直接覆盖 Project Baseline。
...
由用户触发的继续工作笔记。每个任务最多有一个当前 `resume.md`。只有用户运行 `ff-resume` 或明确要求从 resume 继续时才读取。后续第一个成功记录进展的工作流动作会删除它。
...
check 通过后，`ff-work` 会停下来，询问是否运行 `ff-finish`。
...
它不写实现代码。spec 还不清楚时，回到 `ff-clarify` 或阻塞任务。
...
Flowflow 没有单独的 close 或 archive 动作。
...
action-local 自动检查由 preflight 负责。`ff-doctor` 用于更宽的人工诊断。
...
Agent 编排属于执行策略层。它可以改善分工、复核和长任务推进，但工作流语义仍由 `ff-*` 命令、`.ff` 文件和 kernel helper 决定。agent 编排不得直接写入 task state；状态变化必须经过现有 helper。
...
`clarifier` 不作为默认独立角色。澄清需要主会话直接和用户交互；advisor 可以参与 `ff-clarify`，但职责是从旁审查 Proposed Spec 是否遗漏动机、边界、验收标准、风险或更短路径。advisor 的发现回到主会话，由主会话决定是否继续询问用户。

Advisor 采用 OMP 风格：

- 默认启用，配置位于 `.ff/orchestration.json`。
...
支持本地 hook 的 harness 会生成 local watchdog artifact。watchdog 只调用 `ff internal validate-clarify --watchdog`，不复制 gate 逻辑。write `spec.md`、推进 plan、或通过 accept gate 前，validator 失败必须阻断；中间 clarify 状态可以只提醒。
...
- `reasoning_effort` 的 Flowflow 标准值为 `none`、`low`、`medium`、`high`、`xhigh`、`auto`，也可以为 `null` 表示继承平台默认值。`minimal` 当前不进入 Flowflow 标准 schema。
- 平台 adapter 生成的 agent 文件默认继承平台模型，除非 `.ff/orchestration.json` 提供角色默认值或平台覆盖值。
- 用户可以在 `.ff/orchestration.json` 的 `roles.<role>.model` 设置跨平台默认模型。
...
- 平台自身配置优先级仍由平台负责。Flowflow 只生成角色契约和建议默认值。
- advisor、planner、reviewer 默认需要更强推理；implementer、checker 默认需要可靠工具使用；baseline-writer 默认需要低成本、精确改写能力。

支持 harness：

- Claude Code：生成 `.claude/skills/<ff-command>/SKILL.md` 和 `.claude/agents/ff-<role>.md`。
- Codex：生成 `.agents/skills/<ff-command>/SKILL.md` 和 `.codex/agents/ff-<role>.toml`。Codex role agent 使用多行 TOML `developer_instructions`，并在解析后模型非空时写入 `model`，推理 effort 非空时写入 `model_reasoning_effort`。
- OpenCode：生成 `.agents/skills/<ff-command>/SKILL.md` 和 `.opencode/agents/ff-<role>.md`。OpenCode agent 使用项目级 Markdown agent frontmatter：`description`、`mode: subagent`、`model`、可选 `temperature`，以及 `tools.write/edit/bash` 权限。
- Pi：生成 `.agents/skills/<ff-command>/SKILL.md` 和 `.pi/agents/ff-<role>.md`。`ff init --harness pi` 默认执行 `pi install npm:pi-subagents`；用户可以用 `--pi-subagents skipped` 显式跳过。`ff update --harness pi` 只刷新生成文件，不执行安装。
- Cursor：生成 `.agents/skills/<ff-command>/SKILL.md` 和 `.cursor/agents/ff-<role>.md`。
...
1. Flowflow 预览 baseline delta。
2. 用户确认、选择、编辑或跳过。
3. agent 语义编辑 `.ff/project/*`。
...
增强项都可以跳过。没有这些增强，Flowflow 仍然完整可用。
...
Flowflow 会为选定的 coding harness 生成原生文件，不强制所有 harness 共用同一种 frontmatter 格式。
...
1. 运行 `ff init`，生成至少一个 harness 入口。
2. 用 `ff-work` 开始新任务。
3. 用 `ff-clarify` 生成已接受的 `spec.md`。
4. 用 `ff-plan` 创建 `plan.md` 和 `task.md`。
5. 用 `ff-run` 实现 checklist 项。
6. 用 `ff-check` 按任务约定验证和 review。
7. 用 `ff-finish` 通过 Closure Gate 关闭任务。
8. 用 `ff-resume` 处理用户触发的继续工作笔记。
9. 用 `ff-discard` 安全放弃任务。
10. 用 `ff-understand` 为已有仓库生成 Project Baseline 草稿。
11. 用 `ff-doctor` 检查仓库工作流健康状态。
