...
Implement task-local resume notes. A user should be able to explicitly create a concise resume note, later invoke resume for that task, and have Flowflow consume the note without making it the primary source of recovery. Normal recovery should still use task state, trace, and task artifacts.

## Acceptance criteria

- [ ] A task can have at most one current resume note.
- [ ] Resume note creation records the artifact in task state.
- [ ] `ff-resume` reads the resume note and returns the task to normal workflow.
