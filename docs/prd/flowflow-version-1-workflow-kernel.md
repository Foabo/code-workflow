# PRD: Flowflow Version 1 Workflow Kernel

Label: `ready-for-agent`

## Problem Statement

Developers using coding harnesses often lose task state across sessions, repeat context building, let implementation drift away from the task spec, and finish tasks without a clear contract, checklist, verification, or project knowledge update. Existing spec-driven and workflow tools contain useful ideas, but adopting any one framework directly would force Flowflow into someone else's runtime, file model, or harness assumptions.

The user wants Flowflow to be a finished, agent-native workflow tool for coding harnesses: light enough to use by default, strict enough to prevent task rot and false finish, and structured enough to let different coding harnesses share the same repository-local task truth.

## Solution

Build Flowflow version 1 as a TypeScript/Node.js workflow kernel for coding harnesses. Flowflow provides `ff-*` agent commands, deterministic kernel helpers, a minimal Repo Truth, small task artifacts, Project Baseline files, preflight checks, finish closure checks, user-triggered resume notes, discard handling, optional existing-repo understanding, and skippable enhancement configuration.

Users work primarily through agent commands such as `ff-work`, `ff-clarify`, `ff-plan`, `ff-run`, `ff-check`, and `ff-finish`. The CLI remains a thin setup and maintenance surface. Flowflow keeps task truth in small repository-local files, relies on Git for code changes, and promotes stable task learnings into Project Baseline files only when a task finishes.

## User Stories

1. As a developer, I want to initialize Flowflow in a repository, so that coding harnesses can share one task workflow.
...
3. As a developer, I want init to generate selected coding harness entries, so that I can use Flowflow from the harness I already work in.
4. As a developer, I want Flowflow commands to use a `ff-` prefix, so that they do not collide with harness or plugin commands.
...
6. As a developer, I want optional external memory/context detection during init, so that Flowflow can warn me about memory tools without treating them as truth.
...
9. As a developer onboarding an existing project, I want an understand action, so that Flowflow can draft project background from current repository facts.
...
14. As a developer, I want to start or continue a task with `ff-work`, so that one command can advance routine work.
15. As a developer, I want `ff-work` to stop after check and ask before finish, so that task closure and baseline updates remain intentional.
16. As a developer, I want `ff-clarify` to question unclear requirements, so that the task spec is accepted before implementation.
17. As a developer, I want `ff-clarify` to update only the task spec, so that clarification does not accidentally write code or project baseline files.
18. As a developer, I want `ff-plan` to create the implementation approach and checklist, so that implementation has an executable path.
19. As a developer, I want `ff-run` to execute the next checklist items, so that coding work stays tied to the task plan.
20. As a developer, I want `ff-check` to combine verification and review, so that I do not maintain separate verification and review documents for every task.
21. As a developer, I want `ff-check` to update the task checklist, so that finish can rely on one clear execution record.
22. As a developer, I want `ff-check` to detect spec or plan drift, so that implementation changes do not silently invalidate the task.
23. As a developer, I want `ff-finish` to be the only normal way to close a task, so that completed tasks pass a consistent Closure Gate.
...
25. As a developer, I want `ff-discard` to abandon unwanted tasks, so that unfinished work can be removed without pretending it was completed.
...
28. As a developer, I want `ff-resume` to consume a user-created resume note, so that I can intentionally continue from a short handoff point.
...
31. As a developer, I want `ff-doctor` to inspect workflow health, so that stale tasks, malformed state, and adapter drift are visible.
...
46. As a developer, I want Git to remain the source of truth for code changes, so that Flowflow does not maintain a second diff ledger.
47. As a developer, I want finish to inspect dirty worktree state, so that a task is not closed with ambiguous code changes.
48. As a developer, I want unrelated dirty worktree changes to be acknowledged rather than recorded in a change inbox, so that Flowflow stays simple.
...
59. As a developer, I want inline execution to be fully supported, so that Flowflow works even when a harness has no subagent support.
...
64. As a harness adapter author, I want native generated files per harness, so that Flowflow does not force one cross-platform frontmatter format.
...
67. As a Flowflow maintainer, I want TypeScript and Node.js implementation, so that file workflow, CLI, adapters, and tests can be built quickly with type safety.
68. As a Flowflow maintainer, I want public CLI setup and maintenance commands only, so that daily workflow remains agent-native.
69. As a Flowflow maintainer, I want internal helper commands, so that state mutation can be tested independently of natural language behavior.
70. As a Flowflow maintainer, I want schema validation for version and task state records, so that malformed Repo Truth is caught early.
71. As a Flowflow maintainer, I want project validation to be callable from tests and CLI, so that generated structures are externally observable.
72. As a Flowflow maintainer, I want adapter generation tests at the highest seam, so that generated harness files are verified by user-visible outputs.
73. As a Flowflow maintainer, I want workflow action tests at the dispatcher seam, so that a complete task path is verified as behavior.
74. As a Flowflow maintainer, I want finish tests to exercise Closure Gate failure and success, so that tasks cannot be closed by bypassing the workflow.
75. As a Flowflow maintainer, I want discard tests to require confirmation, so that abandoned work is not removed accidentally.
76. As a Flowflow maintainer, I want understand tests to verify drafts, so that existing-repo analysis does not directly overwrite baseline files.
77. As a Flowflow maintainer, I want baseline sync tests to verify accepted delta application, so that stable facts are promoted intentionally.
78. As a Flowflow maintainer, I want self-use artifacts in this repository, so that Flowflow can evolve through the workflow it defines.
...
- Build Flowflow version 1 in TypeScript on Node.js, distributed as an npm package with a public CLI and generated agent-command surfaces.
- Use agent-native `ff-*` commands as the daily invocation surface and keep the public CLI focused on setup, validation, update, and diagnostics.
...
The primary test seam should remain the workflow kernel API and CLI-observable repository outputs. The ideal number of major seams is one: invoke Flowflow at the workflow/action level and assert the resulting Repo Truth, generated files, and user-visible command behavior.
